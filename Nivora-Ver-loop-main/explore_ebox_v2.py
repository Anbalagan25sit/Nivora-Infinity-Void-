    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for line in body_safe.split('\n'):
                line = line.strip()
                if any(x in line for x in ['Solution Of', 'Vector Calculus', 'Analytic Functions', 'Complex Integration', 'Partial Differential']):
            ...
            topic_clicked = False
            try:
                topic = page.get_by_text("Solution Of Ordinary", exact=False).first
                await topic.click(timeout=5000)
                topic_clicked = True
                print("    Clicked 'Solution Of Ordinary Differential'")
            except:
                ...
                for elem in elems:
                    try:
                        ...
                        if 'Solution Of Ordinary' in text and len(text) < 100:
                            await elem.click()
                            topic_clicked = True
