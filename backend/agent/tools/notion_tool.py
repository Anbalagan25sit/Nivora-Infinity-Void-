...
async def create_notion_page(
    ...
    CRITICAL INSTRUCTION: USE THIS NATIVE API TOOL EXCLUSIVELY FOR NOTION. DO NOT USE BROWSER AUTOMATION.
    NOTE: You CANNOT create Notion databases using tools. If the user asks to create a database, use this tool to create a standard page instead, and explicitly TELL THE USER that you created a page because database creation is not supported.
    ...
    try:
        ...
        if parent_page_id:
            ...
            # Auto-discover a parent page if possible
            ...
            if results:
                auto_parent_id = results[0]['id']
                parent = {"type": "page_id", "page_id": auto_parent_id}
            else:
        ...
        try:
            ...
            if "workspace" in str(e):
                return (
                    "I cannot create a page because I don't have access to any parent pages in your Notion workspace. "
                    "Please go to Notion, create a page (or open an existing one), and share it with the Nivora integration."
                )
            raise
...
@function_tool()
async def read_notion_page(page_id_or_url: str) -> str:
    """
...
async def add_to_notion_database(
    ...
    CRITICAL INSTRUCTION: USE THIS NATIVE API TOOL EXCLUSIVELY FOR NOTION. DO NOT USE BROWSER AUTOMATION.
    NOTE: You CANNOT create new databases. You can only add rows to EXISTING databases. If the user asks you to create a new database, explain that they must create it manually in Notion first.
