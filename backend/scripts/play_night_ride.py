import urllib.request
import urllib.parse
import re
import webbrowser
import time
import pyautogui

pyautogui.FAILSAFE = False

def play_night_ride():
    print("Searching YouTube for 'euro truck simulator night ride mr ig'...")
    query = urllib.parse.quote("euro truck simulator night ride mr ig")
    url = f"https://www.youtube.com/results?search_query={query}"
    
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'})
        html = urllib.request.urlopen(req).read().decode('utf-8')
        
        video_ids = []
        for match in re.finditer(r"watch\?v=([a-zA-Z0-9_-]{11})", html):
            vid = match.group(1)
            if vid not in video_ids:
                video_ids.append(vid)
        
        if video_ids:
            latest_video_url = f"https://www.youtube.com/watch?v={video_ids[0]}"
            print(f"Success! Found the video: {latest_video_url}")
            
            webbrowser.open(latest_video_url)
            
            print("Waiting 6 seconds for video to load...")
            time.sleep(6)
            
            print("Pressing 'space' and 'k' to ensure it plays...")
            pyautogui.press('space')
            time.sleep(0.5)
            pyautogui.press('k')
        else:
            print("Could not find a video ID in the search results.")
            
    except Exception as e:
        print(f"Error occurred: {e}")

if __name__ == "__main__":
    play_night_ride()
