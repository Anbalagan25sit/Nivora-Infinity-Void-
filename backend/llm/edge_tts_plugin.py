...
class ChunkedStream(tts.ChunkedStream):
    ...
    async def _run(self, output_emitter: tts.AudioEmitter) -> None:
        """Generate audio using edge-tts with STREAMING for instant response."""
        # Double-check filtering
        text_to_speak = strip_thinking_tags(self._input_text)

        if not text_to_speak.strip():
            logger.warning("Edge TTS: No text to speak after filtering thinking tags. Pushing silent frame to prevent crash.")
            text_to_speak = "hmm"
            self._opts.volume = "-100%"
        ...
        try:
            ...
            # Initialize the emitter IMMEDIATELY (don't wait to buffer)
            output_emitter.initialize(
                request_id=utils.shortuuid(),
                sample_rate=EDGE_TTS_SAMPLE_RATE,
                num_channels=EDGE_TTS_CHANNELS,
                mime_type="audio/mpeg",
                stream=True,  # OPTIMIZED: Enable streaming mode
            )
            ...
            async for chunk in communicate.stream():
                if chunk["type"] == "audio":
                    # Push audio immediately as it arrives
                    output_emitter.push(chunk["data"])
                    chunk_count += 1

                    # Flush every 5 chunks for smoother playback
                    if chunk_count % 5 == 0:
    ...
    async def _run(self, output_emitter: tts.AudioEmitter) -> None:
        ...
        async for data in self._input_ch:
            if isinstance(data, str):
                ...
                volume_opt = self._opts.volume
                if not filtered_text.strip():
                    logger.warning("Edge TTS Streaming: Filtered text is empty. Pushing silent frame.")
                    filtered_text = "hmm"
                    volume_opt = "-100%"
