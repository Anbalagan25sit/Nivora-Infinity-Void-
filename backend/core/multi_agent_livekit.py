...
try:
    ...
    BROWSER_USE_TOOLS_AVAILABLE = True
    logging.info("[MultiAgent] Browser-use AI agent tools loaded")
except ImportError as e:
    logging.warning(f"[MultiAgent] Browser-use tools not available: {e}")
    logging.warning("[MultiAgent] Install with: pip install browser-use langchain langchain-aws")
    BROWSER_USE_TOOLS_AVAILABLE = False
    # Create dummy functions so code doesn't break
    solve_ebox_differential_equations = None
    solve_ebox_specific_section = None
    explain_browser_use_agent = None

# LangGraph-orchestrated browser-use tools (NEW - advanced orchestration!)
try:
    from browser_use_langgraph_tools import (
        solve_ebox_course_langgraph,
        solve_ebox_section_langgraph
    )
    LANGGRAPH_TOOLS_AVAILABLE = True
    logging.info("[MultiAgent] ✨ LangGraph orchestration tools loaded (Claude API + advanced workflow!)")
except ImportError as e:
    logging.warning(f"[MultiAgent] LangGraph tools not available: {e}")
    logging.warning("[MultiAgent] Install with: pip install langgraph langgraph-checkpoint")
    LANGGRAPH_TOOLS_AVAILABLE = False
    solve_ebox_course_langgraph = None
    solve_ebox_section_langgraph = None

# Universal Web Agent tools (powerful web automation)
try:
    from universal_web_tools import (
    ...
    UNIVERSAL_WEB_TOOLS_AVAILABLE = True
    logging.info("[MultiAgent] Universal Web Agent tools loaded (6 powerful automation tools)")
except ImportError as e:
    logging.warning(f"[MultiAgent] Universal Web tools not available: {e}")
    logging.warning("[MultiAgent] Universal Web Agent requires browser-use integration")
    UNIVERSAL_WEB_TOOLS_AVAILABLE = False
    # Create dummy functions so code doesn't break
    automate_website_task = None
    search_and_compare_prices = None
    automate_shopping_cart = None
    automate_form_filling = None
    monitor_website_changes = None
    get_web_agent_performance = None
...
class AgentConfig:
    ...
    # Add browser-use AI agent tools to Nivora (if available)
    ...
    # Add LangGraph orchestration tools to Nivora (if available) - RECOMMENDED!
    if LANGGRAPH_TOOLS_AVAILABLE:
        NIVORA_TOOLS.extend([
            solve_ebox_course_langgraph,
            solve_ebox_section_langgraph,
        ])
        logger.info("[AgentConfig] ✨ LangGraph orchestration enabled for Nivora!")

    # Add Universal Web Agent tools to Nivora (if available)
    if UNIVERSAL_WEB_TOOLS_AVAILABLE:
        NIVORA_TOOLS.extend([
            automate_website_task,
            search_and_compare_prices,
            automate_shopping_cart,
    ...
    # Add select Universal Web Agent tools to Infin for productivity
    if UNIVERSAL_WEB_TOOLS_AVAILABLE:
        INFIN_TOOLS.extend([
            automate_website_task,  # For general web automation
            automate_form_filling,  # For form submissions
            monitor_website_changes,  # For monitoring tasks
        ])
...
async def entrypoint(ctx: JobContext):
    ...
    # Start the background worker loop for proactive task execution
    ...
    # Inject memory facts into the starting prompt context
    if user_facts:
        facts_context = "\\n".join([f"- {k}: {v}" for k, v in user_facts.items()])
        initial_agent.instructions += f"\\n\\n### Personal User Facts From Memory\\n{facts_context}"
