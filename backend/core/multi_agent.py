    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=5) as client:
            ...
            if resp.status_code >= 400:
                logger.warning("MCP server returned HTTP %s — skipping to avoid crash. URL: %s", resp.status_code, N8N_MCP_URL)
                return None
...
async def entrypoint(ctx: JobContext):
    ...
    session = AgentSession(
        ...
        # Increase from the default of 3 to prevent mid-task limit hits.
        # Hitting the limit causes livekit to retry with tool_choice='none',
        # which triggers an AWS Bedrock ValidationException (toolConfig required
        # whenever toolUse/toolResult blocks are in the conversation history).
        max_tool_steps=10,
    )
