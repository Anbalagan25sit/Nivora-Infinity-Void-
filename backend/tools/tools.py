...
try:
    ...
    async def youtube_find_live_streams(*args, **kwargs):
        return "YouTube automation not available."
    async def play_youtube_quick(*args, **kwargs):
        return "YouTube automation not available."

# Import E-Box course automation (removed per user request)
EBOX_TOOLS = []

# Import Notion tools
try:
    from agent.tools.notion_tool import NOTION_TOOLS
    logger.info(f"Notion tools loaded successfully ({len(NOTION_TOOLS)} tools)")
except ImportError as e:
    logger.warning(f"Could not import Notion tools: {e}")
    NOTION_TOOLS = []

# Import browser_harness tools
try:
    from browser_harness_tool import HARNESS_TOOLS
    logger.info(f"browser-harness tools loaded successfully ({len(HARNESS_TOOLS)} tools)")
except ImportError as e:
    logger.warning(f"Could not import browser-harness tools: {e}")
    HARNESS_TOOLS = []
...
@function_tool()
async def get_current_cryptocurrency_price(coin_name: str) -> str:
    """ Get current price of Bitcoin, Ethereum, or other popular cryptocurrencies"""
    try :
        coin = coin_name.strip().lower()
        response = urllib.request.urlopen(f"https://api.coingecko.com/api/v3/simple/price?ids={coin}&vs_currencies=usd")
        raw_data = response.read()
        data = json.loads(raw_data)
        if coin in data :
            return f"The current price of {coin_name} is ${round(data[coin]['usd'],2)}"
    except Exception as e:
        return f"I couldn't find the price for {coin_name}."

@function_tool()
async def web_search(context: RunContext, query: str) -> str:
    """Search the web for current events, news, and facts."""
    try:
        import requests
        ...
        for s in snippets[:5]:
            ...
            results.append(f"- {title}: {snippet_text} ({link})")
...
async def spotify_play(context: RunContext, query: str = "") -> str:
    ...
    If the API fails because Spotify is not active, it will automatically fallback to the local desktop app.
    """
    if not query:
        success = spotify_api.play()
        if success:
            return "Resumed playback (API)."
        else:
            return "Failed to resume playback via API. Ensure Spotify is open."

    # First, try to get the exact track URI to avoid UI fallback hallucinations
    exact_uri = None
    try:
        data = spotify_api.search(query, type="track", limit=1)
        if data and data.get("tracks") and data.get("tracks", {}).get("items"):
            exact_uri = data["tracks"]["items"][0].get("uri")
            logger.info(f"Found exact Spotify URI for query '{query}': {exact_uri}")
    except Exception as e:
        logger.warning(f"Failed to get exact URI for Spotify search: {e}")

    success, msg = spotify_api.search_and_play_track(query)

    if not success and ("No active device" in msg or "not active" in msg.lower() or "Is Spotify open" in msg):
        logger.info(f"Spotify API failed due to no active device. Falling back to local app: {query}")
        try:
            import os
            import subprocess
            import sys
            ...
            if exact_uri:
                logger.info(f"Executing local Spotify macro for exact track URI: {exact_uri}")
                # Direct URIs (track, album, artist) auto-play, no need for UI macros
                result = subprocess.run(
                    [sys.executable, SPOTIFY_CONTROL_SCRIPT, 'search', '--uri', exact_uri],
                    capture_output=True,
                    text=True,
                    check=False
                )
                if result.returncode != 0:
                    return f"Spotify API had no active device. Tried to launch desktop app with exact track URI but it failed."
                return f"Spotify API wasn't active, so I launched the desktop app and played the exact track '{query}'!"
            else:
                # Step 2: Use our local script to bypass the API entirely!
                # The spotify_control.py script uses the spotify:search: URI and Windows UI macros
                # to literally type and click play, requiring zero API connection
                encoded_query = urllib.parse.quote(query)
                uri = f"spotify:search:{encoded_query}"

                logger.info(f"Executing local Spotify macro for search: {uri}")
                result = subprocess.run(
                ...
                # Let's add a direct PyAutoGUI failsafe here just in case the macro script fails
                # Based on the screenshot, it looks like it gets stuck with the search bar focused
                # To select the top result "Top Result" card from the search bar:
                # Tab 1 time (moves to the Top Result card) -> Enter (starts playing)
                pyautogui.FAILSAFE = False

                # Make sure Spotify is in the foreground
                os.startfile(uri)
                await asyncio.sleep(2)

                logger.info("Running direct PyAutoGUI fallback just in case...")
                # Click near top-middle to make sure window is focused
                w, h = pyautogui.size()

                # The safest, most reliable way to play the top result in Spotify is to
                # literally click the big green Play button on the "Top Result" card.
                # In a standard maximized 1080p Spotify window, the Top Result play button
                # is around x=650, y=250.
                # We'll click slightly to the left of the center just to be sure we hit the card.

                logger.info("Clicking the Big Green Play button directly...")
                # Click the big green play button on the album/track page
                # Usually located at ~28% width from the left and ~45% height from the top.
                click_x = int(w * 0.28)
                click_y = int(h * 0.45)

                pyautogui.click(click_x, click_y)
                await asyncio.sleep(0.2)
                pyautogui.click(click_x, click_y) # double click to start it!

                # Alternate shortcut for play/pause in Spotify if it has focus: Space
                await asyncio.sleep(0.5)
                pyautogui.press('enter')
                pyautogui.press('space')

                if result.returncode != 0:
                    return f"Spotify API had no active device. I opened the app to your search and tried to hit play."

                return f"Spotify API wasn't active, so I launched the desktop app and used a local macro to play '{query}'!"
        except Exception as fallback_err:
            return f"Failed to play via API ({msg}) and fallback also failed ({fallback_err})."
    ...
    try:
        if "pause" in cmd:
            ...
            try:
                ...
                if "No active device" in str(e) or "not active" in str(e).lower():
                    import pyautogui
                    pyautogui.press("nexttrack")
                    return "Skipped to next track locally since no active API device."
                return str(e)
            ...
            try:
                ...
                if "No active device" in str(e) or "not active" in str(e).lower():
                    import pyautogui
                    pyautogui.press("prevtrack")
                    return "Went back locally since no active API device."
                return str(e)
        elif "shuffle" in cmd:
...
async def open_spotify(
    ...
    """
    Open the Spotify desktop app directly using the spotify: protocol.
    If the app is closed, this will launch it.
    """
    import os
    import urllib.parse
    try:
        if mode == "home":
            ...
            os.startfile(f"spotify:{mode}:{content_id}")
            return f"Opened Spotify {mode} {content_id}."
        else:
            os.startfile("spotify:")
            return "Opened Spotify."
    except Exception as e:
...
async def play_youtube_video(context: RunContext, query: str) -> str:
    ...
    Use this IMMEDIATELY when the user asks to navigate to YouTube, search, or play a video.
    ...
    - "navigate to youtube, search for [query], and play the video"
    """
    import subprocess as _sp
    import pygetwindow as _gw

    async def _click_play_bg(delay: float = 4.0):
        """Background: wait for YouTube to load, then click the video center."""
        await asyncio.sleep(delay)
        try:
            wins = [w for w in _gw.getAllWindows() if w.title and "youtube" in w.title.lower()]
            if wins:
                w = wins[0]
                w.activate()
                await asyncio.sleep(0.4)
                pyautogui.click(w.left + w.width // 2, w.top + w.height // 2 - 40)
                logger.info(f"[YouTube] Auto-clicked play in '{w.title}'")
        except Exception as _e:
            logger.warning(f"[YouTube] Auto-click play failed: {_e}")

    def _launch(url: str) -> None:
        """Open URL in Chrome/Edge with autoplay flag, schedule background click."""
        _candidates = [
        ...
        for _exe in _candidates:
            if os.path.isfile(_exe):
                _sp.Popen(
                    [_exe, "--autoplay-policy=no-user-gesture-required", url],
                    creationflags=_sp.DETACHED_PROCESS | _sp.CREATE_NEW_PROCESS_GROUP,
                )
                break
        else:
            webbrowser.open(url)
        # Fire-and-forget click after page loads (covers "Chrome already running" case)
        try:
            asyncio.get_event_loop().create_task(_click_play_bg(delay=4.0))
        except RuntimeError:
            pass
    ...
    try:
        ...
        if len(q) == 11 and " " not in q and q.isalnum():
            url = f"https://www.youtube.com/watch?v={q}&autoplay=1"
            _launch(url)
            return f"Opened YouTube video ID '{q}' and started playback."

        # Robust API-less Search using urllib and regex
        encoded = urllib.parse.quote(q)
        search_url = f"https://www.youtube.com/results?search_query={encoded}"
        logger.info(f"[YouTube] Fetching search results from: {search_url}")

        def fetch_html(url):
            req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
            return urllib.request.urlopen(req).read().decode('utf-8')

        html = await asyncio.to_thread(fetch_html, search_url)
        ...
        if video_ids:
            ...
            _launch(url)
            logger.info(f"[YouTube] Found and playing video: {url}")
            return f"Playing '{q}' on YouTube at {url}."
        else:
            return f"Could not find any YouTube videos for '{q}'."
...
async def send_email(context: RunContext, recipient: str, subject: str, body: str) -> str:
    ...
    CRITICAL INSTRUCTION: USE THIS NATIVE API TOOL EXCLUSIVELY FOR GMAIL. DO NOT USE BROWSER AUTOMATION.
    """
...
async def google_sheets_read(context: RunContext, sheet_name: str) -> str:
    ...
    CRITICAL INSTRUCTION: USE THIS NATIVE API TOOL EXCLUSIVELY FOR GOOGLE SHEETS. DO NOT USE BROWSER AUTOMATION.
    """
    try:
...
async def ecommerce_price_compare(
    ...
    try:
        ...
        async with use_global_browser(backend="playwright", visual_mode=False) as browser:
...
async def open_app(app_name: str) -> str:
    """
    Opens a specific Windows application.
    Supported: notepad, calculator, paint, taskmgr
    """
    import os  # You learned about imports!

      # We use a dictionary to map names to actual Windows commands
      # You learned about Dictionaries in your course!
    apps = {
        "notepad": "notepad.exe",
        "calculator": "calc.exe",
        "paint": "mspaint.exe",
    ...
    if app_lower in apps:
          # os.system runs a command in your Windows terminal
        os.system(f"start {apps[app_lower]}")
        return f"I've opened {app_name} for you."
    else:
        return f"I'm sorry, I don't know how to open '{app_name}' yet."
@function_tool()
...
async def get_bitcoin_price() -> str :
    ...
    try :
        #Open url
        # Use the general currentprice endpoint
        response = urllib.request.urlopen("https://api.coindesk.com/v1/bpi/currentprice.json")
        raw_data = response.read()
        #Change web folder into a dictionary
        data = json.loads(raw_data)
        #Get the price of bitcoin in USD as a float, not a string
        price = data["bpi"]["USD"]["rate_float"]
        return f"The current price of Bitcoin is ${round(price, 2)}"
    except Exception as e :
...
# ============================================================================
# BROWSER-USE AI AGENT TOOLS (Autonomous E-Box Solver)
# ============================================================================
try:
    ...
    logger.info("[Tools] Browser-use AI agent tools loaded (1 tool)")
except ImportError as e:
    logger.warning(f"[Tools] Browser-use tools not available: {e}")
    logger.warning("[Tools] Install with: pip install browser-use langchain langchain-aws")
    BROWSER_USE_TOOLS = []
...
# ============================================================================
# DEEP RESEARCH TOOLS (Reddit, HackerNews, Multi-Source Research)
# ============================================================================

try:
    from deep_research_tools import (
        deep_web_research,
    ...
    logger.info("[Tools] Deep Research tools loaded (4 powerful research tools)")
except ImportError as e:
    logger.warning(f"[Tools] Deep Research tools not available: {e}")
    logger.warning("[Tools] Deep Research requires: duckduckgo-search, beautifulsoup4, boto3")
    DEEP_RESEARCH_TOOLS = []
...
ALL_TOOLS = [
    ...
    open_website,
    get_current_cryptocurrency_price,
    web_search,
    get_weather,
    system_control,
    get_current_time,
    ...
    # NOTE: spotify_play, spotify_control, spotify_shortcut, spotify_search removed from here
    # They are provided by ALL_SPOTIFY_TOOLS from spotify_tools_advanced.py
    ...
    music_intent_router,
] + ALL_SPOTIFY_TOOLS + SOCIAL_TOOLS + BROWSER_USE_TOOLS + DEEP_RESEARCH_TOOLS + NOTION_TOOLS + HARNESS_TOOLS # Add all advanced tools including Deep Research, and Harness
